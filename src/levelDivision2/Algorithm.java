                ...
                while (meta_task.size() != 0){
                    ...
                    while(it.hasNext()){
                        ...
                        for(int i=0;i<tasks.length;i++){
                            temp[i] = c[tasks[i].intValue()][p]*(cmax[tasks[i].intValue()]/tmax[tasks[i].intValue()]);
                        }
                        argTask = argSort(temp);

                        sum[p] = 0;
                        for(int i=0;i<k;i++){
                            sum[p] += c[tasks[argTask[i]]][p]*(cmax[tasks[argTask[i]]]/tmax[tasks[argTask[i]]]);
                        }
